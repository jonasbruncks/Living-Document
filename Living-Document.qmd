...
# Datenaufbereitung
data <- data(open(("data_combined.csv")))

# ATI-Skala berechnen (Items 1-9, einige invertieren)
ati_items <- data %>% select(starts_with("ati_"))
ati_items <- ati_items %>% mutate(across(c(ati_3, ati_6, ati_7), ~6 - .))
data$ati_score <- rowMeans(ati_items, na.rm = TRUE)

# Deskriptive Statistiken
desc_stats <- data %>% 
  select(age, ati_score, b_dlr_wat_a, ki_dlr_wat_a) %>%
  describe() %>%
  select(mean, sd, min, max, n, median)

# Cronbach's Alpha für ATI
alpha_ati <- psych::alpha(ati_items)$total$raw_alpha

# Tabelle erstellen
desc_table <- desc_stats %>%
  kable(digits = 2,
        caption = "Deskriptive Statistiken der Hauptvariablen",
        col.names = c("M", "SD", "Min", "Max", "n", "Median")) %>%
  kable_styling()

desc_table

![Diagramm](DiagrammquantitativeStudie.drawio){width=300}
